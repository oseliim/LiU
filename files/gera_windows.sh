...
cp chroot_scripts/vmware.sh "$CHROOT_DIR"
chmod +x $CHROOT_DIR/vmware.sh

cat > $CHROOT_DIR/usr/local/bin/start-vmware-safe.sh <<EOF
#!/bin/bash
modprobe vmmon
modprobe vmnet
exec vmplayer -X ~/vmware/Ubuntu/Ubuntu.vmx
EOF
chmod +x $CHROOT_DIR/vmware.sh
...
cat > /etc/xdg/autostart/vmplayer.desktop <<'EOF'
[Desktop Entry]
Type=Application
Name=VMware Player
Exec=sh -c '/usr/local/bin/start-vmware-safe.sh'
