...
echo "[expresso] Criando 30 usuários alunos_labconf01 a alunos_labconf30 como sudoers"
USER_DATA_FILE="/tmp/user_data.txt"
mkdir -p /tmp
> "$USER_DATA_FILE"
#Criando usuários, senha, nome e quantidade
for i in $(seq -w 1 30); do
    USUARIO="alunos_labconf${i}"
    SENHA="aluno"
    if id "$USUARIO" &>/dev/null; then
        echo "$USUARIO:$SENHA" | chpasswd
        grep -v "^${USUARIO}:" "$USER_DATA_FILE" > "${USER_DATA_FILE}.tmp" || true
...
for i in $(seq -w 1 30); do
    user="alunos_labconf${i}"
    grep -q "AUTOLOGIN=${user}" "$ltsp_conf_file" && continue
    while (( ip_suffix <= range_end )); do
        ip="${network_prefix}.${ip_suffix}"
        grep -q "^\[${ip//./\\.}\]" "$ltsp_conf_file" || break
        ((ip_suffix++))
    done
    [[ $ip_suffix -gt $range_end ]] && break
